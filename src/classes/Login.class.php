...
class Login {
  ...
  public function isPasswordValid($uname = '', $pw = '', $pwnew1 = '', $pwnew2 = '') {
    ...
    $result = 0;
    if (empty($uname)) $result = 10;
    if (empty($pwnew1) || empty($pwnew2)) $result = 11;
    if ($pwnew1 != $pwnew2) $result = 12;
    //
    // MINIMUM LENGTH
    //
    if (strlen($pwnew1) < $this->min_pw_length) $result = 20;
    if ($this->pw_strength > 0) {
      ...
      if (preg_match($uname_lower, $pwnew1_denum)) $result = 30;
      if (preg_match(strrev($uname_lower), $pwnew1_denum)) $result = 31;
      if ($pwnew1_lower == $pw_lower) $result = 32;
      if ($this->pw_strength > 1) {
        //
        // MEDIUM STRENGTH
        //
        if (!preg_match('[0-9]', $pwnew1)) $result = 40;

        if ($this->pw_strength > 2) {
          //
          // HIGH STRENGTH
          //
          if (!preg_match('[a-z]', $pwnew1)) $result = 50;
          if (!preg_match('[A-Z]', $pwnew1)) $result = 51;
          if (!preg_match('[^a-zA-Z0-9]', $pwnew1)) $result = 52;
        }
      }
    }
    return $result;
  ...
  private function ldapVerify($uidpass) {
    ...
    //
    if (!@ldap_bind($ds)) $result = 93;

    //
    // Start TLS
    //
    if ($ldaptls && !ldap_start_tls($ds)) $result = 94;

    //
    // LDAP Search bind
    // (patch by Franz Gregor)
    //
    if (!@ldap_bind($ds, $ldaprdn, $ldappass)) $result = 96;

    //
    // Search for user UID
    //
    if (LDAP_ADS) {
      if (!$info = ldap_first_entry($ds, ldap_search($ds, $searchbase, "sAMAccountName=" . $U->username, $attr))) $result = 95;
    } else {
      if (!$info = ldap_first_entry($ds, ldap_search($ds, $searchbase, "uid=" . $U->username, $attr))) $result = 95;
    }
  ...
  public function login($loginname = '', $loginpwd = '') {
    ...
    if (empty($loginname) || empty($loginpwd)) return 1;
    ...
    if ($U->onhold && ($now - $U->grace_start <= $this->grace_period)) return 6; // Login is locked for this account and grace period is not over yet.
