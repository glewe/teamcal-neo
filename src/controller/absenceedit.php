...
if (!empty($_POST)) {
    ...
    if (!$inputError) {
        ...
        if (isset($_POST['btn_save'])) {
            ...
            if (isset($_POST['txt_symbol'])) $AA->symbol = $_POST['txt_symbol'];
            else $AA->symbol = strtoupper(substr($_POST['txt_name'], 0, 1));
            if (isset($_POST['opt_iconcolor'])) $AA->iconcolor = $_POST['opt_iconcolor'];
            else $AA->iconcolor = 'default';
            $AA->color = $_POST['txt_color'];
            $AA->bgcolor = $_POST['txt_bgcolor'];
            if (isset($_POST['chk_bgtrans'])) $AA->bgtrans = '1';
            else $AA->bgtrans = '0';
            ...
            if (isset($_POST['chk_counts_as_present'])) $AA->counts_as_present = '1';
            else $AA->counts_as_present = '0';
            if (isset($_POST['chk_approval_required'])) $AA->approval_required = '1';
            else $AA->approval_required = '0';
            if (isset($_POST['chk_manager_only'])) $AA->manager_only = '1';
            else $AA->manager_only = '0';
            if (isset($_POST['chk_hide_in_profile'])) $AA->hide_in_profile = '1';
            else $AA->hide_in_profile = '0';
            if (isset($_POST['chk_confidential'])) $AA->confidential = '1';
            else $AA->confidential = '0';
            if (isset($_POST['chk_takeover'])) $AA->takeover = '1';
            else $AA->takeover = '0';
            if (isset($_POST['chk_show_in_remainder'])) $AA->show_in_remainder = '1';
            else $AA->show_in_remainder = '0';
...
if ($viewData['counts_as']['val']) $viewData['counts_as']['name'] = $AA->getName($AA->counts_as);
else $viewData['counts_as']['name'] = "None";
...
foreach ($groups as $group) {
    if ($AG->isAssigned($viewData['id'], $group['id'])) $selected = true;
    else $selected = false;
