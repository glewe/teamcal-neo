...
if (isset($_POST['btn_caloptApply'])) {
    ...
    if (isset($_POST['chk_showWeekNumbers']) && $_POST['chk_showWeekNumbers']) $C->save("showWeekNumbers", "1");
    else $C->save("showWeekNumbers", "0");
    $C->save("repeatHeaderCount", intval($_POST['txt_repeatHeaderCount']));
    $C->save("usersPerPage", intval($_POST['txt_usersPerPage']));
    if (isset($_POST['chk_showAvatars']) && $_POST['chk_showAvatars']) $C->save("showAvatars", "1");
    else $C->save("showAvatars", "0");
    if (isset($_POST['chk_showRoleIcons']) && $_POST['chk_showRoleIcons']) $C->save("showRoleIcons", "1");
    else $C->save("showRoleIcons", "0");
    if (isset($_POST['chk_showTooltipCount']) && $_POST['chk_showTooltipCount']) $C->save("showTooltipCount", "1");
    else $C->save("showTooltipCount", "0");
    if (isset($_POST['chk_supportMobile']) && $_POST['chk_supportMobile']) $C->save("supportMobile", "1");
    else $C->save("supportMobile", "0");
    if (isset($_POST['chk_symbolAsIcon']) && $_POST['chk_symbolAsIcon']) $C->save("symbolAsIcon", "1");
    else $C->save("symbolAsIcon", "0");
    if ($_POST['sel_monitorAbsence']) $C->save("monitorAbsence", $_POST['sel_monitorAbsence']);
    else $C->save("monitorAbsence", 0);
    if (strlen($_POST['txt_calendarFontSize'])) $C->save("calendarFontSize", intval($_POST['txt_calendarFontSize']));
    else $C->save("calendarFontSize", 100);
    ...
    // Filter
    //
    if (isset($_POST['chk_hideDaynotes']) && $_POST['chk_hideDaynotes']) $C->save("hideDaynotes", "1");
    else $C->save("hideDaynotes", "0");
    if (isset($_POST['chk_hideManagers']) && $_POST['chk_hideManagers']) $C->save("hideManagers", "1");
    else $C->save("hideManagers", "0");
    if (isset($_POST['chk_hideManagerOnlyAbsences'])) $C->save("hideManagerOnlyAbsences", "1");
    else $C->save("hideManagerOnlyAbsences", "0");
    if (isset($_POST['chk_showUserRegion']) && $_POST['chk_showUserRegion']) $C->save("showUserRegion", "1");
    else $C->save("showUserRegion", "0");
    ...
    if (isset($_POST['chk_satBusi']) && $_POST['chk_satBusi']) $C->save("satBusi", "1");
    else $C->save("satBusi", "0");
    if (isset($_POST['chk_sunBusi']) && $_POST['chk_sunBusi']) $C->save("sunBusi", "1");
    else $C->save("sunBusi", "0");
    if ($_POST['sel_defregion']) $C->save("defregion", $_POST['sel_defregion']);
    else $C->save("defregion", "default");
    if (isset($_POST['chk_showRegionButton']) && $_POST['chk_showRegionButton']) $C->save("showRegionButton", "1");
    else $C->save("showRegionButton", "0");
    if ($_POST['opt_defgroupfilter']) $C->save("defgroupfilter", $_POST['opt_defgroupfilter']);
    else $C->save("defgroupfilter", 'All');
    if (isset($_POST['chk_currentYearOnly']) && $_POST['chk_currentYearOnly']) $C->save("currentYearOnly", "1");
    else $C->save("currentYearOnly", "0");
    ...
    if (isset($_POST['chk_takeover']) && $_POST['chk_takeover']) $C->save("takeover", "1");
    else $C->save("takeover", "0");
    if (isset($_POST['chk_notificationsAllGroups']) && $_POST['chk_notificationsAllGroups']) $C->save("notificationsAllGroups", "1");
    else $C->save("notificationsAllGroups", "0");
    if (isset($_POST['chk_managerOnlyIncludesAdministrator']) && $_POST['chk_managerOnlyIncludesAdministrator']) $C->save("managerOnlyIncludesAdministrator", "1");
    else $C->save("managerOnlyIncludesAdministrator", "0");

    //
    // Statistics
    //
    if ($_POST['sel_statsDefaultColorAbsences']) $C->save("statsDefaultColorAbsences", $_POST['sel_statsDefaultColorAbsences']);
    else $C->save("statsDefaultColorAbsences", "red");
    if ($_POST['sel_statsDefaultColorPresences']) $C->save("statsDefaultColorPresences", $_POST['sel_statsDefaultColorPresences']);
    else $C->save("statsDefaultColorPresences", "green");
    if ($_POST['sel_statsDefaultColorAbsencetype']) $C->save("statsDefaultColorAbsencetype", $_POST['sel_statsDefaultColorAbsencetype']);
    else $C->save("statsDefaultColorAbsencetype", "cyan");
    if ($_POST['sel_statsDefaultColorRemainder']) $C->save("statsDefaultColorRemainder", $_POST['sel_statsDefaultColorRemainder']);
    else $C->save("statsDefaultColorRemainder", "orange");

    //
    // Summary
    //
    if (isset($_POST['chk_includeSummary']) && $_POST['chk_includeSummary']) $C->save("includeSummary", "1");
    else $C->save("includeSummary", "0");
    if (isset($_POST['chk_showSummary']) && $_POST['chk_showSummary']) $C->save("showSummary", "1");
    else $C->save("showSummary", "0");
