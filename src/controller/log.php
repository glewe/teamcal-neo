...
$sort = "DESC";
if (isset($_GET['sort']) && strtoupper($_GET['sort']) == "ASC") $sort = "ASC";
$eventsPerPage = 50;
$currentPage = 1;
  ...
  if (!$inputError) {
    ...
    elseif (isset($_POST['btn_logSave'])) {
      foreach ($logtypes as $lt) {
        ...
        // Set log colors
        //
        if ($_POST['opt_logcolor' . $lt]) $C->save("logcolor" . $lt, $_POST['opt_logcolor' . $lt]);
        else $C->save("logcolor" . $lt, "default");
      }
      //
      // Log this event
      //
...
$viewData['sort'] = $sort;
if ($viewData['numEvents']) $viewData['numPages'] = ceil($viewData['numEvents'] / $viewData['eventsPerPage']);
else $viewData['numPages'] = 0;
