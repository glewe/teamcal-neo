...
if (!empty($_POST)) {
    ...
    if (!$inputError) {
        ...
        if (isset($_POST['btn_apply'])) {
            ...
            if (isset($_POST['chk_showAsPieChart']) && $_POST['chk_showAsPieChart']) $viewData['showAsPieChart'] = true;
            else $viewData['showAsPieChart'] = false;
        }
    } else {
        //
...
if (count($labels) <= 10) $viewData['height'] = count($labels) * 20;
else $viewData['height'] = count($labels) * 10;
