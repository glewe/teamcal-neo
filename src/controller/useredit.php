  ...
  if (!$inputError) {
    ...
    if (isset($_POST['btn_profileUpdate'])) {
      ...
      if (isAllowed("useraccount")) {
        ...
        if (isset($_POST['chk_onhold']) && $_POST['chk_onhold']) {
          $UP->onhold = '1';
        } else {
          $UP->onhold = '0';
        }
      ...
      if (isset($_POST['chk_remove2fa']) && $_POST['chk_remove2fa']) {
        $UO->deleteUserOption($profile, 'secret');
      }
      $UP->update($profile);
      //
...
//
$selected = false;
if (!$UO->read($viewData['profile'], 'calfilterGroup') || $UO->read($viewData['profile'], 'calfilterGroup') == 'All') {
  $selected = true;
}
...
foreach ($groups as $group) {
  if ($UG->isMemberOrManagerOfGroup($viewData['profile'], $group['id'])) {
    $selected = false;
    if ($UO->read($viewData['profile'], 'calfilterGroup') == $group['id']) {
      $selected = true;
    }
...
if (!$UO->read($profile, 'region')) {
  $UO->save($profile, 'region', '1');
}
$viewData['regionList'][] = array( 'val' => "1", 'name' => "Default", 'selected' => ($UO->read($profile, 'region') == "1") ? true : false );
foreach ($regions as $region) {
  if ($region['id'] != 1) {
    $viewData['regionList'][] = array( 'val' => $region['id'], 'name' => $region['name'], 'selected' => ($UO->read($profile, 'region') == $region['id']) ? true : false );
  }
}
