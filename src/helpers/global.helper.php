  ...
  if (count($myExt) || $myPrefix) {
    ...
    if (count($files)) {
      foreach ($files as $pos => $file) {
        ...
        if (
          count($myExt) && !$myPrefix && in_array($thisExt, $myExt) ||
          (!count($myExt) && $myPrefix && startsWith($thisPref, $myPrefix)) ||
          (count($myExt) && $myPrefix && (in_array($thisExt, $myExt) && startsWith($thisPref, $myPrefix)))
        ) {
          $filearray[] = $file;
        }
      }
...
function getLanguages($type = 'app') {
  ...
  if ($fileidx > 0) {
    ...
    for ($i = 0; $i < $fileidx; $i++) {
      ...
      if (($type == 'log' && $langName[1] == 'log') || $langName[1] == 'php') {
