  ...
  <div class="col-lg-12">
    ...
    <form class="form-control-horizontal" enctype="multipart/form-data" action="index.php?action=<?= $controller ?>" method="post" target="_self" accept-charset="utf-8">
      ...
      <div class="card">
        ...
        ?>
        <div class="card-header text-bg-<?= $CONF['controllers'][$controller]->panelColor ?>"><i class="<?= $CONF['controllers'][$controller]->faIcon ?> fa-lg me-3"></i><?= $LANG['att_title'] ?><?= $pageHelp ?></div>
        <div class="card-body">
                  ...
                  <?php foreach ($viewData['uplFiles'] as $file) {
                    ...
                    <div class="row" style="border-bottom: 1px dotted; margin-bottom: 10px; padding-bottom: 10px;">
                      <div class="col-lg-5">
                        ...
                        <?php if (in_array($file['ext'], $CONF['imgExtensions'])) { ?>
                        ...
                        <?php } else { ?>
                          <a href="<?= APP_UPL_DIR . $file['fname'] ?>"><img src="images/icons/mimetypes/<?= $file['ext'] ?>.png" alt="" style="width: 24px; height: 24px;"></a>
                        <?php } ?>
                        <?= $file['fname'] ?>
                      </div>
                      <div class="col-lg-2">
