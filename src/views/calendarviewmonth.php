  ...
  for ($t = 0; $t < $tables; $t++) {
    ...
    <div class="table<?= ($viewData['supportMobile']) ? $key : ''; ?>">
      <table class="table table-bordered month">
        ...
        <?php require_once "calendarviewmonthheader.php"; ?>
            ...
            foreach ($viewData['users'] as $usr) {
              if (count($viewData['groups']) == 1) {
                ...
                if ($repeatHeaderCount && $rowcount > $repeatHeaderCount) {
                  require_once "calendarviewmonthheader.php";
                  $rowcount = 1;
                }
                if (!$groupHeader) { ?>
                  <!-- Row: Group <?= $grp['name'] ?> -->
                  <tr>
                    <th id="th-<?= $grp['name'] ?>" class="m-groupname" colspan="<?= $days + 1 ?>"><?= $grp['description'] . ' (' . $grp['name'] . ')' ?></th>
                  </tr>
                ...
                <?php require_once "calendarviewuserrow.php";
                if ($repeatHeaderCount) {
                  $rowcount++;
                }
              } else {
