...
if (isset($_POST['btn_confApply']))
{
   ...
   if ($_POST['sel_welcomeIcon']) $C->save("welcomeIcon", $_POST['sel_welcomeIcon']); else $C->save("welcomeIcon", "None");
   $C->save("welcomeText", htmlspecialchars(addslashes(strip_tags($_POST['txt_welcomeText'], "<em><strong>"))));
   $C->save("welcomeTitle", htmlspecialchars(addslashes(strip_tags($_POST['txt_welcomeTitle'], "<em><strong>"))));
...
{
   $configData['languageList'][] = array ('val' => $appLang, 'name' => proper($appLang), 'selected' => ($C->read("defaultLanguage") == $appLang)?true:false );
}

foreach ($logLanguages as $logLang)
{
   $configData['logLanguageList'][] = array ('val' => $logLang, 'name' => proper($logLang), 'selected' => ($C->read("logLanguage") == $logLang)?true:false );
}
...
{
   $configData['schemeList'][] = array ('val' => $scheme, 'name' => $scheme, 'selected' => ($C->read("permissionScheme") == $scheme)?true:false );
}

$configData['welcomeIconList'][] = array ('val' => "None", 'name' => "None", 'selected' => ($C->read("welcomeIcon") == "None")?true:false );

foreach ($appWelcomeIcons as $welcomeIcon)
{
   $configData['welcomeIconList'][] = array ('val' => $welcomeIcon, 'name' => $welcomeIcon, 'selected' => ($C->read("welcomeIcon") == $welcomeIcon)?true:false );
}

$configData['general'] = array (
   array ( 'prefix' => 'config', 'name' => 'appTitle', 'type' => 'text', 'value' => urldecode($C->read("appTitle")), 'maxlength' => '160' ),
   array ( 'prefix' => 'config', 'name' => 'defaultLanguage', 'type' => 'list', 'values' => $configData['languageList'] ),
   array ( 'prefix' => 'config', 'name' => 'logLanguage', 'type' => 'list', 'values' => $configData['logLanguageList'] ),
   ...
   array ( 'prefix' => 'config', 'name' => 'permissionScheme', 'type' => 'list', 'values' => $configData['schemeList'] ),
   array ( 'prefix' => 'config', 'name' => 'userManual', 'type' => 'text', 'value' => urldecode($C->read("userManual")), 'maxlength' => '160' ),
   array ( 'prefix' => 'config', 'name' => 'homepage', 'type' => 'radio', 'values' => array ('home', 'news'), 'value' => $C->read("homepage") ),
   array ( 'prefix' => 'config', 'name' => 'welcomeTitle', 'type' => 'text', 'value' => $C->read("welcomeTitle"), 'maxlength' => '160' ),
   array ( 'prefix' => 'config', 'name' => 'welcomeIcon', 'type' => 'list', 'imagelist' => true, 'imagedir' => $CONF['app_homepage_dir'], 'values' => $configData['welcomeIconList'], 'action' => array ( 'name' => 'upload', 'target' => 'index.php?action=upload&amp;target=homepage') ),
   array ( 'prefix' => 'config', 'name' => 'welcomeText', 'type' => 'textarea', 'value' => $C->read("welcomeText"), 'rows' => '10' ),
   array ( 'prefix' => 'config', 'name' => 'appFooterCpy', 'type' => 'text', 'value' => $C->read("appFooterCpy"), 'maxlength' => '160' ),
);

$configData['email'] = array (
...
{
   $configData['jqueryUIThemeList'][] = array ('val' => $jqueryUITheme, 'name' => proper($jqueryUITheme), 'selected' => ($C->read("jqtheme") == $jqueryUITheme)?true:false );
}
foreach ($timezones as $tz)
{
   $configData['timezoneList'][] = array ('val' => $tz, 'name' => $tz, 'selected' => ($C->read("timeZone") == $tz)?true:false );
}
$configData['system'] = array (
   ...
   array ( 'prefix' => 'config', 'name' => 'jqtheme', 'type' => 'list', 'values' => $configData['jqueryUIThemeList'] ),
   array ( 'prefix' => 'config', 'name' => 'timeZone', 'type' => 'list', 'values' => $configData['timezoneList'] ),
...
{
   $configData['themeList'][] = array ('val' => $appTheme, 'name' => proper($appTheme), 'selected' => ($C->read("theme") == $appTheme)?true:false );
}

$configData['theme'] = array (
   array ( 'prefix' => 'config', 'name' => 'theme', 'type' => 'list', 'values' => $configData['themeList'] ),
...
$configData['user'] = array (
   ...
   array ( 'prefix' => 'config', 'name' => 'avatarWidth', 'type' => 'text', 'value' => $C->read("avatarWidth"), 'maxlength' => '3' ),
   array ( 'prefix' => 'config', 'name' => 'avatarHeight', 'type' => 'text', 'value' => $C->read("avatarHeight"), 'maxlength' => '3' ),
   array ( 'prefix' => 'config', 'name' => 'avatarMaxSize', 'type' => 'text', 'value' => $C->read("avatarMaxSize"), 'maxlength' => '7' ),
